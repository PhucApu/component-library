# Recreate Text Field as three files
...
- `text-field.js` is one ES module holding the DOM-free rules, the custom element, and the demo
  bootstrap. It defines the element only when it is not already registered.
- `text-field.css` holds every style, driven by component-owned CSS custom properties.
- `text-field.html` is a runnable example showing a plain field, a validated field, an adorned
  field, and a multiline field, reporting values into an `<output>`.
- `README.md` documents the markup contract, validation, the attribute table, and browser support.

Use `lang="en"`. ES modules do not load from `file://`, so state in the README that the page must
be served over HTTP or HTTPS, while noting the inputs themselves would still work.
...
Use `:user-invalid` for the visual state and set `aria-invalid` on the same terms. Mark the field
interacted on blur and on the native `invalid` event, and let `validate()` force the state for a
submit handler.
...
- A required field is not red on load, turns invalid after you leave it empty, and clears as soon
  as you fix it.
- A `pattern` field shows your own message rather than the browser's.
- The counter changes as you type but your screen reader does not read it out each time.
- Submitting a form carries the read-only value and omits the disabled one.
- Turn scripting off: the fields still accept text and the form still submits.
- Clicking the label focuses the field, and clicking the reveal button does not.
- Exactly one focus indicator is visible when a field has focus.
